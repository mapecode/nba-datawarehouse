[![uclm](https://img.shields.io/badge/uclm-esi-red.svg?logo=data:image/png;base64,&longCache=true&colorA=b30135&colorB=555555&style=for-the-badge)](https://www.esi.uclm.es)  
# NBA DataWareHouse
Este trabajo se basa en la creación de un almacén de datos
de la NBA, con el objetivo de obtener datos relevantes sobre la temporada
2018-2019. Hemos construido un almacén de datos para almacenar
las estadísticas mas relevantes.

Más información en la [documentación](Memoria.pdf)
